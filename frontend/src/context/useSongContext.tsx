...
export const SongProvider: React.FC<SongProviderProps> = ({ children }) => {
  ...
  const fetchSongs = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`${server}/api/v1/song/all`);
  ...
  const fetchSingleSong = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${server}/api/v1/song/${selectedSong}`);
      const data = await response.json();
      setSong(data);
    } catch (error) {
      console.log(error);
  ...
  const fetchAlbums = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`${server}/api/v1/album/all`);
      const data = await response.json();
      setAlbums(data);
    } catch (error) {
      console.log(error);
  ...
  const fetchAlbumsongs = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch album ${id}: ${response.status} ${response.statusText}`);
      }
      const data: { songs: Song[]; album: Album } = await response.json();
